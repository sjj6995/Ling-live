...
//		initData();
	...
	private void initData() {
		if (CommonUtil.checkNetState(getActivity())) {
			...
			HttpClient.post(JFConfig.GET_WEALTH_AND_USERAMOUNT, params, new AsyncHttpResponseHandler() {
				@Override
			...
			HttpClient.post(JFConfig.HOME_PAGE, params, new AsyncHttpResponseHandler() {
				...
				public void onSuccess(int statusCode, String content) {
					...
					if ("true".equals(commonData.getCommonData().getReturnStatus())) {
						...
						List<DataSaveEntity> tempEntities = db.queryAll(JFConfig.FRONT_PAGE);
						fillData(wealthEntities, tempEntities);
						gridview.setAdapter(new WealthAdapter(getActivity(), wealthEntities));
	...
	protected void fillData(List<WealthEntity> wealthEntities, List<DataSaveEntity> tempEntities) {
